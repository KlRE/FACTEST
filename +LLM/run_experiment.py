...
def run_experiment(num_iterations=30, description="", model='mistral-nemo'):
    ...
    for env in Env:
        successful = iterative_prompt(env.value, num_iterations, model=model, directory=path)
        log_results_file.write(f"{env.value}: {successful}\n")

    log_results_file.close()


if __name__ == "__main__":
    run_experiment(description="First experiment")
